...
/**
	template <int D, typename E>
  	class BaseEdge 					: D->Dimension, E->Measurement

	template <int D, typename E, typename VertexXi>
  	class BaseUnaryEdge : public BaseEdge<D,E> 			: : D->Dimension, E->Measurement
*/
...
int main(int argc, char** argv){
	...
	for(int i = 0; i < numPoints; i++){
		double r = g2o::Sampler::gaussRand(radius, 0.1);
	...
	*/
	...
	cout << "radius of the circle " << circle->estimate()(2) << endl;
	cout << "error" << errorOfSolution(numPoints, points, circle->estimate()) << endl << endl;

	

	/** 作为对比，下面给出线性最小二乘方法的计算结果 **/
	...
	// where c = a^2 + b^2 - r^2.
	// Since we have a bunch of points, we accumulate Eqn S(1) in a matrix and
	...
	// calculate the radius of the circle given the solution so for
